...
const UpdateResourcesCostModal = ({
  isOpen,
  onClose,
  onSave,            // function passed from parent to handle saving & updating
  initialResources = []
}) => {
  const [resources, setResources] = useState([]);
  const [loading, setLoading] = useState(false); // 🔹 loading state

  // Load previous resources when modal opens
  useEffect(() => {
    if (isOpen) {
      setResources(
        initialResources.length
          ? initialResources.map(r => ({ name: r.name, cost: r.cost })) // clone to prevent direct mutation
          : [{ name: "", cost: "" }]
      );
    }
  }, [isOpen]);
  ...
  const totalCost = useMemo(
    () => resources.reduce((sum, r) => sum + (Number(r.cost) || 0), 0),
    [resources]
  );
  ...
  const updateResource = (index, field, value) => {
    ...
    updated[index][field] = value; // keep as string until validation
  ...
  const handleSave = async () => {
    ...
    for (let i = 0; i < resources.length; i++) {
      const r = resources[i];
      if (!r.name.trim()) {
        toast.error(`Resource ${i + 1} name cannot be empty`);
        return;
      }
      if (r.cost === "" || Number(r.cost) <= 0) {
        toast.error(`Resource ${i + 1} cost must be greater than 0`);
        return;
      }
    }
    ...
    try {
      // Await parent save function (e.g., API call)
      await onSave(validResources, totalCost);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="flex items-center gap-3 sm:gap-2">
          ...
          <div>
            <h2 className="text-xl font-bold sm:text-lg dark:text-white">Update Resources & Cost</h2>
            <p className="text-sm text-gray-500 sm:text-xs">
              Update the resources used and their costs for this resolved complaint.
            </p>
          </div>
        </div>
          ...
          {resources.map((res, index) => (
            ...
            >
              <div className="col-span-12 sm:col-span-7">
                <label className="text-sm font-medium dark:text-gray-200">Resource Name</label>
                <input
                  type="text"
                  value={res.name}
              ...
              <div className="col-span-12 sm:col-span-4">
                <label className="text-sm font-medium dark:text-gray-200">Cost (PKR)</label>
                <input
                  type="number"
                  value={res.cost}
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button
            onClick={onClose}
            className="cursor-pointer flex-1 bg-gray-200 px-5 py-3 rounded-lg"
            disabled={loading}
          >
            Cancel
          </button>
...
export default UpdateResourcesCostModal;
